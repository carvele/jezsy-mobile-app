...
// GCash is what was asked for; the others cost nothing to list, and PayMongo
// only offers what the merchant account has actually activated.
const PAYMENT_METHODS = ["gcash", "grab_pay", "card"];
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: reservation, error: reservationError } = await admin
      .from("reservations")
      .select("id, customer_id, deposit, display_id, product_name, payment_status")
      .eq("id", reservationId)
      .maybeSingle();
