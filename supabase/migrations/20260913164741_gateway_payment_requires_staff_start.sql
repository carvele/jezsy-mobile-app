-- ============================================================================
-- Migration: gateway payments require staff to start preparing, and the
-- rejected-receipt retry window is a flat reset instead of a floor.
--
-- 1. settle_payment_webhook auto-advanced status straight to 'Preparing' the
--    instant PayMongo confirmed payment, skipping the same staff checkpoint
--    a manually-uploaded receipt already goes through. The admin board
--    already has a guarded "Start preparing" action for a paid reservation
--    still sitting in 'To Pay' (see transition_reservation_status and
--    reservationActions.primaryActionFor in admin-dashboard) -- it was just
--    unreachable for gateway payments because the webhook bypassed it.
--    Fix: mark payment_status = 'Paid' only; leave status untouched so staff
--    make the same explicit call for every payment method.
--
-- 2. review_reservation_receipt's reject branch extended payment_due_at with
--    GREATEST(existing_due_at, now() + retry_minutes). GREATEST is a floor,
--    not a cap: rejecting a receipt early in a fresh window (e.g. 10 minutes
--    into a 2-hour manual window) left the *original*, longer deadline in
--    place, so the customer could get well over an hour to retry instead of
--    the intended 30-60 minute window. Fix: flat-reset to
--    now() + retry_minutes every time.
-- ============================================================================
...
BEGIN
  ...
  IF _next_status = 'paid' AND v_payment.reservation_id IS NOT NULL THEN
    -- status is intentionally left untouched here: a reservation still awaiting
    -- payment stays at whatever status it already had (typically 'To Pay') so
    -- staff must explicitly "Start preparing" once payment shows Paid, the same
    -- checkpoint a manually-reviewed receipt already goes through.
