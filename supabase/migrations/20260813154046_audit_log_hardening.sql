-- Audit logging for sensitive actions (role changes, balance settlement,
...
-- a UI bug that throws before logAction fires) leaves zero trace, and
-- (2) balance settlement (settle_reservation_balance) was never logged at
-- all, client or server. This migration makes all three guaranteed at the
...
BEGIN
  ...
  VALUES (
    v_actor, COALESCE(v_actor_name, 'Staff'), 'Changed staff role',
...
BEGIN
  ...
  -- Sever the same identity link on the thread itself, not just its
  -- messages -- otherwise a conversation stays permanently resolvable to a
  -- specific (scrubbed) customer even though every message inside it lost
  -- its sender.
  ...
  -- Logged after the scrub above, with the request id as the target (not
  -- the now-nulled user_id) -- this row is the durable record that the
  -- deletion happened, independent of the PII it just removed.
  INSERT INTO public.logs (user_id, user_name, action, target_type, target_id, details)
  VALUES (
    v_actor, COALESCE(v_actor_name, 'Staff'), 'Processed account deletion request',
    'account_deletion_request', _request_id::text,
    jsonb_build_object('deleted_user_id', v_request.user_id)
  );
