...
-- the owner dashboard's device-approval gate (fingerprint, status,
...
--
-- Verified before writing this migration: owner-dashboard's
...
-- on an authenticated session, so gating devices to staff/owner does not
...
-- ("Enable all access for owner/staff", TO authenticated, USING/WITH CHECK
