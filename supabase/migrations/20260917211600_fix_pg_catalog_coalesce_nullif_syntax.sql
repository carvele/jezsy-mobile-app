-- Migration: 20260917211600_fix_pg_catalog_coalesce_nullif_syntax.sql
-- Description: 20260917211500_harden_rpc_search_path_and_settings_rls.sql
-- schema-qualified every bare identifier under SET search_path = '' hardening,
-- including COALESCE and NULLIF as if they were ordinary pg_catalog functions
-- (pg_catalog.coalesce(...), pg_catalog.nullif(...)). They are not -- COALESCE
-- and NULLIF are SQL-standard special forms parsed directly by the grammar,
-- not functions resolvable via schema-qualified lookup, so every call raised
-- "function pg_catalog.coalesce(...) does not exist". This broke all 5 RPCs
-- that migration touched: is_reservation_return_eligible,
-- cancel_customer_reservation, request_customer_refund,
-- review_return_refund_request, mark_reservation_refund_disbursed -- meaning
-- cancellation, return/refund requests, staff review, and refund disbursement
-- were all broken the moment that migration landed. Confirmed live: a
-- customer return/refund request failed with exactly this error.
-- Fix: use bare COALESCE(...)/NULLIF(...) (search_path-independent, so
-- hardening is unaffected) everywhere those two were schema-qualified.
-- Every other pg_catalog-qualified call (trim, lower, now, sum, substr,
-- jsonb_build_object, jsonb_typeof, round) is a real catalog function and is
-- untouched.
...
BEGIN
  ...
  IF pg_catalog.lower(pg_catalog.trim(COALESCE(v_res.status, ''))) <> 'completed' THEN
    RETURN false;
  END IF;
...
BEGIN
  ...
  v_status := pg_catalog.lower(pg_catalog.trim(COALESCE(v_res.status, '')));
  v_pstatus := pg_catalog.lower(pg_catalog.trim(COALESCE(v_res.payment_status, '')));
  ...
  IF EXISTS (
    ...
    WHERE p.reservation_id = _reservation_id
      AND pg_catalog.lower(pg_catalog.trim(COALESCE(p.status, ''))) IN ('paid', 'processing')
  ) THEN
  ...
  SET status = 'Cancelled',
      cancellation_reason = COALESCE(NULLIF(pg_catalog.trim(_reason), ''), 'Cancelled by customer'),
...
DECLARE
  ...
  v_trimmed_reason text := pg_catalog.trim(COALESCE(_reason_category, ''));
  v_display text;
BEGIN
  IF v_actor IS NULL THEN
    RAISE EXCEPTION 'Authentication required.' USING ERRCODE = '42501';
  ...
  VALUES (
    ...
    v_actor,
    v_trimmed_reason,
    NULLIF(pg_catalog.trim(_details), ''),
    NULLIF(pg_catalog.trim(_photo_path), ''),
    'submitted'
  )
  RETURNING * INTO v_req;

  v_display := COALESCE(v_res.display_id, pg_catalog.substr(_reservation_id::text, 1, 8));
...
DECLARE
  ...
  v_req public.return_refund_requests%rowtype;
  v_res public.reservations%rowtype;
  v_normalized_decision text := pg_catalog.lower(pg_catalog.trim(COALESCE(_decision, '')));
  v_prev_status text;
  v_new_status text;
BEGIN
  ...
  IF v_normalized_decision = 'approve' THEN
    ...
    SET status = 'approved',
        ...
        reviewed_by = v_actor,
        resolution_notes = NULLIF(pg_catalog.trim(_notes), ''),
        updated_at = pg_catalog.now()
    ...
    SET requires_refund = true,
        refund_required_at = COALESCE(refund_required_at, pg_catalog.now()),
        updated_at = pg_catalog.now()
    WHERE reservation_id = v_req.reservation_id
      AND status = 'paid'
      AND refund_disbursed_at IS NULL;
  ...
  ELSIF v_normalized_decision = 'reject' THEN
    IF pg_catalog.trim(COALESCE(_notes, '')) = '' THEN
  ...
  ELSIF v_normalized_decision = 'under_review' THEN
    ...
    SET status = 'under_review',
        under_review_at = COALESCE(under_review_at, pg_catalog.now()),
        under_review_by = COALESCE(under_review_by, v_actor),
        resolution_notes = NULLIF(pg_catalog.trim(_notes), ''),
        updated_at = pg_catalog.now()
    WHERE id = _request_id;
  END IF;
...
BEGIN
  ...
  IF pg_catalog.lower(COALESCE(v_reservation.status, '')) = 'completed' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.return_refund_requests
      WHERE reservation_id = _reservation_id
        AND status = 'approved'
      ...
      RAISE EXCEPTION 'Refund disbursement for a completed reservation requires an approved return/refund request.'
        USING ERRCODE = 'check_violation';
    END IF;
  ELSIF pg_catalog.lower(COALESCE(v_reservation.status, '')) <> 'cancelled' THEN
  ...
  IF pg_catalog.lower(COALESCE(v_reservation.payment_status, '')) = 'refunded'
    AND NOT FOUND
  THEN
    RETURN pg_catalog.jsonb_build_object(
  ...
  SELECT COALESCE(pg_catalog.sum(amount_centavos), 0)
