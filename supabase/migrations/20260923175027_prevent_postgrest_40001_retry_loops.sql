-- =============================================================================
-- Migration: 20260923175027_prevent_postgrest_40001_retry_loops.sql
--
-- Concurrency Conflict Contract Hardening & Database Health Stabilization:
-- 1. Standardizes optimistic-concurrency stale state conflict exceptions to use
--    ERRCODE = 'PT409' across all reservation manager RPCs.
-- 2. Permanently retires SQLSTATE 40001 (serialization_failure) for business
--    validation mismatches, preventing repeated transaction execution loops in PostgREST.
-- 3. Cleans stale comments referencing serialization_failure.
-- 4. Preserves 100% of current live production business logic.
-- =============================================================================

-- 1. transition_reservation_status
...
-- 2. reschedule_reservation_as_manager
...
REVOKE ALL ON FUNCTION public.reschedule_reservation_as_manager(uuid, text, date, time without time zone, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reschedule_reservation_as_manager(uuid, text, date, time without time zone, text) TO authenticated;


-- 3. cancel_reservation_as_manager
