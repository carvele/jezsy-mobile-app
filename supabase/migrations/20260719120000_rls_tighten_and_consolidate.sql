...
--      owner-or-owner policy. RLS policies are OR-ed, so the broad policy
...
--      for auditability; behavior is unchanged (owner-or-owner).
...
CREATE POLICY "Enable read for own profile or owner"
  ON public.profiles
  FOR SELECT
  USING (auth.uid() = id OR is_staff_or_admin());

-- 2. wardrobe_items: drop the overly-broad policy, keep owner-or-owner
DROP POLICY IF EXISTS "Enable all access for authenticated users" ON public.wardrobe_items;

-- 3. saved_outfits: drop the overly-broad policy, keep owner-or-owner
...
-- "Enable select/insert/update-delete for own measurements or owner" policies
-- already cover owner-or-owner for all commands; nothing further to create.
