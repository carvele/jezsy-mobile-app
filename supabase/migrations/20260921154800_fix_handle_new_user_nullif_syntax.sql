-- Migration: 20260921154800_fix_handle_new_user_nullif_syntax.sql
-- Description: Fix invalid pg_catalog.nullif and pg_catalog.coalesce calls in handle_new_user()
-- and record_signup_legal_acceptance(). COALESCE and NULLIF are SQL syntax constructs parsed
-- directly by the grammar, not schema-qualified functions in pg_catalog. Attempting to call
-- pg_catalog.nullif(...) caused "function pg_catalog.nullif(text, unknown) does not exist (SQLSTATE 42883)"
-- during auth.users signup insertion, resulting in HTTP 500 "Database error saving new user".
