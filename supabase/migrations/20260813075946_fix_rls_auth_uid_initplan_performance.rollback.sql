...
CREATE POLICY "Allow admins to create categories" ON public.categories AS PERMISSIVE FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'owner'::text])) AND (profiles.deleted = false) AND (profiles.is_blocked = false)))));
...
CREATE POLICY "Allow admins to update categories" ON public.categories AS PERMISSIVE FOR UPDATE TO public USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'owner'::text])) AND (profiles.deleted = false) AND (profiles.is_blocked = false))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::text, 'owner'::text])) AND (profiles.deleted = false) AND (profiles.is_blocked = false)))));
...
CREATE POLICY "Enable insert for own conversation or admin" ON public.conversations AS PERMISSIVE FOR INSERT TO public WITH CHECK (((customer_id = auth.uid()) OR is_staff_or_admin()));

DROP POLICY IF EXISTS "Enable select for own conversation or admin" ON public.conversations;
CREATE POLICY "Enable select for own conversation or admin" ON public.conversations AS PERMISSIVE FOR SELECT TO public USING (((customer_id = auth.uid()) OR is_staff_or_admin()));

DROP POLICY IF EXISTS "Enable update for own conversation or admin" ON public.conversations;
CREATE POLICY "Enable update for own conversation or admin" ON public.conversations AS PERMISSIVE FOR UPDATE TO public USING (((customer_id = auth.uid()) OR is_staff_or_admin())) WITH CHECK (((customer_id = auth.uid()) OR is_staff_or_admin()));
...
CREATE POLICY "Enable insert for messages in own conversation or admin" ON public.messages AS PERMISSIVE FOR INSERT TO public WITH CHECK ((is_staff_or_admin() OR ((sender_id = auth.uid()) AND (EXISTS ( SELECT 1
...
CREATE POLICY "Enable select for messages in own conversation or admin" ON public.messages AS PERMISSIVE FOR SELECT TO public USING (((sender_id = auth.uid()) OR is_staff_or_admin() OR (EXISTS ( SELECT 1
   FROM conversations
...
CREATE POLICY "Enable read for own profile or admin" ON public.profiles AS PERMISSIVE FOR SELECT TO public USING (((auth.uid() = id) OR is_staff_or_admin()));
...
CREATE POLICY "Enable select for own reservations or admin" ON public.reservations AS PERMISSIVE FOR SELECT TO public USING (((customer_id = auth.uid()) OR is_staff_or_admin()));
...
CREATE POLICY "Enable all access for own saved outfits or admin" ON public.saved_outfits AS PERMISSIVE FOR ALL TO public USING (((user_id = auth.uid()) OR is_staff_or_admin())) WITH CHECK (((user_id = auth.uid()) OR is_staff_or_admin()));
...
CREATE POLICY "History readable by staff and admin" ON public.staff_status_history AS PERMISSIVE FOR SELECT TO public USING (((( SELECT profiles.role
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.deleted = false))) = ANY (ARRAY['admin'::text, 'owner'::text])) OR (staff_id = auth.uid())));
...
CREATE POLICY "Enable insert for own measurements or admin" ON public.user_measurements AS PERMISSIVE FOR INSERT TO public WITH CHECK (((user_id = auth.uid()) OR is_staff_or_admin()));

DROP POLICY IF EXISTS "Enable select for own measurements or admin" ON public.user_measurements;
CREATE POLICY "Enable select for own measurements or admin" ON public.user_measurements AS PERMISSIVE FOR SELECT TO public USING (((user_id = auth.uid()) OR is_staff_or_admin()));

DROP POLICY IF EXISTS "Enable update/delete for own measurements or admin" ON public.user_measurements;
CREATE POLICY "Enable update/delete for own measurements or admin" ON public.user_measurements AS PERMISSIVE FOR ALL TO public USING (((user_id = auth.uid()) OR is_staff_or_admin())) WITH CHECK (((user_id = auth.uid()) OR is_staff_or_admin()));
...
CREATE POLICY "Enable all access for own wardrobe items or admin" ON public.wardrobe_items AS PERMISSIVE FOR ALL TO public USING (((user_id = auth.uid()) OR is_staff_or_admin())) WITH CHECK (((user_id = auth.uid()) OR is_staff_or_admin()));
