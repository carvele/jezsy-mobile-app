-- Returns to pay-at-creation (the 20260730051910 behaviour).
--
-- Reservations already sitting Pending with payment_due_at NULL will never be
-- swept after this runs, because the pay-first sweep requires a deadline that
-- only creation sets. Stamp or cancel those by hand if this is rolled back
-- with any in flight.
...
-- Restores the 120-minute-at-creation stamp.
create or replace function public.create_reservation(
  _product_id uuid, _size text, _color text, _quantity integer,
  _date text, _appointment_time text, _receipt_path text,
