...
--   * Owner dashboard (github.com/carvele/owner-dashboard) does insert into
...
--   * Owner dashboard never touches orders or order_items at all (only renders
...
-- ── reservations ────────────────────────────────────────────────────────────
-- Customer keeps SELECT on their own rows; writes become owner-only.
...
DROP POLICY IF EXISTS "Enable all access for own orders or owner" ON public.orders;
...
DROP POLICY IF EXISTS "Enable all access for own order items or owner" ON public.order_items;
