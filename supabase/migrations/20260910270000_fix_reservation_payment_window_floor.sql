-- Fixes a real gap: booking a same-day slot close enough to "now" could
-- produce a payment_due_at that was already at or past "now" the moment the
-- reservation was created -- confirmed live (reservation RES-1A08A54E3F7-872,
-- created 2026-09-10 16:00:14 Manila for a 16:00:00 appointment the same
-- day: payment_due_at ended up 16:00:00, already 14 seconds in the past at
-- INSERT time). The reservation still held stock until the expiry cron
-- eventually cancelled it -- a dead-on-arrival booking with zero real
-- window to pay.
--
-- Root cause: least(now() + 24h, appointment - 1h), with only a same-value
-- fallback to the appointment time itself if that's already past "now" --
-- which can itself still be at or behind "now" by the time the INSERT
-- actually runs. There was no floor guaranteeing any minimum window, and no
-- cap tighter than 24h for far-future appointments either.
--
-- New formula, used identically in both places a payment_due_at is
-- (re)computed:
--   LEAST(appointment - 30 minutes, now() + 5 hours), floored at now() + 2 hours.
-- Always at least 2h to pay, never longer than 5h held unpaid, and normally
-- ends 30 min before the appointment so paying and pickup don't collide.
-- The floor can only push payment_due_at past the appointment itself if the
-- appointment is under 2h30m away -- paired with a matching 150-minute
-- minimum lead time in the mobile client's TimeSlotPicker (a slot under 2h30m
-- out is no longer offered as bookable at all), so that combination can't
-- actually occur through the app. It is still expressed as an unconditional
-- floor here, not contingent on the client behaving -- this function is the
-- actual boundary, not the picker.
...
BEGIN
  ...
  -- Always at least 2h to pay, never more than 5h held unpaid, normally 30
  -- min before pickup. See migration header for the full reasoning.
  v_payment_due_at := LEAST(v_appointment - interval '30 minutes', now() + interval '5 hours');
  IF v_payment_due_at < now() + interval '2 hours' THEN
    v_payment_due_at := now() + interval '2 hours';
  END IF;
...
BEGIN
  ...
  IF _approve THEN
    ...
    IF public.is_awaiting_payment_status(v_res.status)
      ...
      -- Same floor/cap as create_reservation_multi -- this path previously had
      -- no fallback at all if the requested slot was too close to "now",
      -- worse than the original create-time bug since staff approving a
      -- reschedule from the admin dashboard has no client-side lead-time
      -- guard on that screen.
      v_new_due_at := LEAST(v_res.reschedule_requested_at_time - interval '30 minutes', now() + interval '5 hours');
      IF v_new_due_at < now() + interval '2 hours' THEN
        v_new_due_at := now() + interval '2 hours';
      END IF;
    END IF;
