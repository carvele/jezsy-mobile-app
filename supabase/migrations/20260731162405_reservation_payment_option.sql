-- Lets the customer settle the full price up front instead of the 50%
-- reservation fee. The amount stays server-resolved: the client sends only
-- which option it wants, never a figure, for the same reason the RPC already
-- resolves price from the product row rather than trusting the request.
--
-- Dropped and recreated rather than replaced because the argument list
-- changes. _payment_option is defaulted, so the existing 7-argument callers
-- (and PostgREST calls that omit it) keep working unchanged.

drop function if exists public.create_reservation(uuid, text, text, integer, text, text, text);
...
BEGIN
  ...
  -- deposit holds whatever is due now, so paying in full simply makes that
  -- the whole price. Staff read payment_type to know if a balance remains.
...
-- Same grants the previous definition carried: customers call this, and the
-- PUBLIC default grant has to go explicitly or the REVOKE from anon no-ops.
