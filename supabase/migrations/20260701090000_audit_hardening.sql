-- Audit hardening: reservation consistency, private receipts, atomic checkout,
-- and high-value constraints/indexes.

-- Payment receipts must not be publicly readable.
...
-- Keep the app-facing receipt column aligned with the existing generated types.
ALTER TABLE public.reservations
ADD COLUMN IF NOT EXISTS receipt_url text;

-- Reservation validation now treats date as a calendar day and appointment_time
-- as canonical HH:mm:ss, excludes inactive reservations from capacity, and
-- handles custom closure hours safely.
...
DROP TRIGGER IF EXISTS trg_validate_reservation_time ON public.reservations;
...
-- Atomic checkout RPC. It calculates server-side prices and rolls back both the
-- order and order_items if any item fails.
...
-- Replace the previous public SECURITY DEFINER streak helper with an invoker
-- function that can only mutate the caller's own streak row.
DO $$
BEGIN
...
-- Policies needed by the checkout RPC and order confirmation screen.
...
-- Indexes for common app filters, joins, and RLS policy predicates.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes
    WHERE schemaname = 'public'
      AND indexname = 'orders_display_id_uidx'
  )
  AND NOT EXISTS (
    SELECT 1
    FROM public.orders
    ...
    CREATE UNIQUE INDEX orders_display_id_uidx
...
CREATE INDEX IF NOT EXISTS reservations_customer_deleted_created_idx
ON public.reservations (customer_id, deleted, created_at DESC);
...
BEGIN
  IF to_regclass('public.capsules') IS NOT NULL THEN
    CREATE INDEX IF NOT EXISTS capsules_user_created_idx
    ON public.capsules (user_id, created_at DESC);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS products_public_catalog_idx
ON public.products (visibility, deleted, category, sub_category, created_at DESC);
...
BEGIN
  IF to_regclass('public.wishlists') IS NOT NULL THEN
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS wishlists_user_product_uidx
  ...
  IF to_regclass('public.user_measurements') IS NOT NULL THEN
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS user_measurements_user_id_uidx
    ON public.user_measurements (user_id);
  END IF;
END $$;

-- New-row validation constraints. NOT VALID avoids failing deployment because
-- of legacy data while still enforcing future writes.
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'orders_total_amount_nonnegative'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_measurements_confidence_range'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'store_hours_valid_times'
  ) THEN
    ALTER TABLE public.store_hours
    ADD CONSTRAINT store_hours_valid_times
    CHECK (is_closed OR open_time < close_time) NOT VALID;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'store_closures_valid_custom_hours'
  ) THEN
