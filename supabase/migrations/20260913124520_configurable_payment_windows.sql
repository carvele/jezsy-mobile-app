-- Reservation payment window policy, made configurable and re-tuned per
-- boutique-inventory review:
--   - Manual (bank/e-wallet receipt) max hold: was hardcoded 5h, now 120min
--     default, configurable. A scarce single-unit variant should not be
--     locked most of an afternoon for a payment that typically resolves
--     much faster.
--   - Floor (minimum time to pay): was hardcoded 2h, now 90min, hardcoded
--     (not one of the three tunable values, same as before -- it was
--     always an implementation detail, not a policy knob). Also now
--     clamped so it can never push payment_due_at PAST the appointment
--     itself, regardless of the mobile client's own lead-time guard --
--     the DB is the actual boundary, the client guard is a courtesy.
--   - PayMongo/gateway max hold: was hardcoded 2h in expire_stale_payments,
--     now 30min default, configurable.
--   - Rejected-receipt retry window: did not exist at all -- a reject only
--     reset payment_status, never extended payment_due_at, so a receipt
--     rejected near the original deadline left the customer almost no
--     time before the next expiry-cron pass auto-cancelled them. Now
--     extends payment_due_at to at least now() + retry_minutes (60min
--     default), never shortening whatever time was already left.
--
-- All three tunable values live in one settings row so they can be
-- adjusted without a migration once real completion-time data exists.
INSERT INTO public.settings (key, value)
VALUES (
  'reservationPaymentWindow',
  jsonb_build_object(
    'manual_max_minutes', 120,
    'gateway_max_minutes', 30,
    'retry_minutes', 60
  )
)
...
BEGIN
  ...
  -- Configurable manual-payment ceiling (settings.reservationPaymentWindow,
  -- default 120min), floored at 90min, and never later than the
  -- appointment itself regardless of client-side lead-time enforcement.
  -- See migration header for the full reasoning.
...
BEGIN
  ...
  IF _approve THEN
    ...
    IF public.is_awaiting_payment_status(v_res.status)
      ...
      -- Same floor/cap/settings as create_reservation_multi -- this path
      -- previously had no fallback at all if the requested slot was too
      -- close to "now", worse than the original create-time bug since
      -- staff approving a reschedule from the admin dashboard has no
      -- client-side lead-time guard on that screen.
      v_manual_max_minutes := coalesce(
        (SELECT (value->>'manual_max_minutes')::integer FROM public.settings WHERE key = 'reservationPaymentWindow'),
        120
      );
...
DECLARE
  v_expired integer;
  v_gateway_max_minutes integer;
BEGIN
  v_gateway_max_minutes := coalesce(
    (SELECT (value->>'gateway_max_minutes')::integer FROM public.settings WHERE key = 'reservationPaymentWindow'),
    30
  );
  ...
  WITH stale AS (
    ...
    WHERE status IN ('awaiting_payment', 'processing')
      AND coalesce(attempt_started_at, created_at) < now() - make_interval(mins => v_gateway_max_minutes)
...
CREATE OR REPLACE FUNCTION public.review_reservation_receipt(_reservation_id uuid, _approve boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  ELSE
    -- A rejected receipt is a correctable mistake, not a cancellation: the
    -- customer should get a real window to submit a valid proof rather than
    -- inheriting whatever was left of the ORIGINAL payment_due_at (which
    -- could be minutes away, or already past, handing them straight to the
    -- next expiry-cron pass). Extends to at least now() + retry_minutes
    -- (settings.reservationPaymentWindow, default 60min); never shortens
    -- whatever time was genuinely still left.
