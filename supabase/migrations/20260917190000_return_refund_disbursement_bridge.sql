-- Migration: 20260917190000_return_refund_disbursement_bridge
-- Description:
-- 1. Adds under_review_at and under_review_by to return_refund_requests for full lifecycle timeline persistence.
-- 2. Hardens public.review_return_refund_request with explicit row locking (PERFORM ... FOR UPDATE),
--    prerequisite refundable payment verification, server-side rejection reason validation,
--    and atomic handoff into financial liability (payment_status = 'Refund Required', payments.requires_refund = true).
-- 3. Hardens public.mark_reservation_refund_disbursed with row locking, dual lifecycle support
--    (Completed reservations strictly require an approved return_refund_request; Cancelled reservations preserved),
--    atomic transition of return_refund_requests to 'refunded', and idempotent retry safety.
--
-- Security Role Note:
-- Database roles are: 'customer', 'staff', 'owner'.
-- public.is_staff_or_admin() returns true for ('staff', 'owner') -> Review authority.
-- public.is_admin_or_owner() returns true for ('owner') -> Disbursement authority.

-- ============================================================================
-- 1. Add under_review tracking columns to return_refund_requests
-- ============================================================================
ALTER TABLE public.return_refund_requests
  ADD COLUMN IF NOT EXISTS under_review_at timestamptz,
  ADD COLUMN IF NOT EXISTS under_review_by uuid REFERENCES public.profiles(id);

-- ============================================================================
-- 2. Hardened public.review_return_refund_request
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_normalized_decision = 'reject' THEN
    ...
    -- Invariant: reservation payment_status and payments.requires_refund remain strictly unchanged
...
-- ============================================================================
-- 3. Hardened public.mark_reservation_refund_disbursed
-- ============================================================================
...
BEGIN
  ...
  IF lower(coalesce(v_reservation.status, '')) = 'completed' THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'Refund disbursement for a completed reservation requires an approved return/refund request.'
        USING ERRCODE = 'check_violation';
    END IF;
  ELSIF lower(coalesce(v_reservation.status, '')) <> 'cancelled' THEN
