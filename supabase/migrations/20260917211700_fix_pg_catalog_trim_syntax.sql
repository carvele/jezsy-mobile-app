-- Migration: 20260917211700_fix_pg_catalog_trim_syntax.sql
-- Description: The previous fix (20260917211600_fix_pg_catalog_coalesce_nullif_syntax.sql)
-- corrected pg_catalog.coalesce/pg_catalog.nullif but missed the same bug in
-- pg_catalog.trim(text). TRIM(...) is the SQL-standard special form (the
-- grammar internally maps it to btrim/ltrim/rtrim); there is no real catalog
-- function literally named "trim" that a schema-qualified pg_catalog.trim(...)
-- call can resolve to, so every one of the 21 pg_catalog.trim(...) call sites
-- across the same 5 RPCs raised "function pg_catalog.trim(text) does not
-- exist". Confirmed live: after applying the coalesce/nullif fix, a return/
-- refund request still failed, now with this error instead.
-- Fix: bare TRIM(...) everywhere it was pg_catalog.trim(...). Every other
-- pg_catalog-qualified call (lower, now, sum, substr, jsonb_build_object,
-- jsonb_typeof, round) is a real catalog function and is untouched.
...
BEGIN
  ...
  IF pg_catalog.lower(TRIM(COALESCE(v_res.status, ''))) <> 'completed' THEN
    RETURN false;
  END IF;
...
DECLARE
  ...
  v_req public.return_refund_requests%rowtype;
  v_res public.reservations%rowtype;
  v_normalized_decision text := pg_catalog.lower(TRIM(COALESCE(_decision, '')));
  v_prev_status text;
  v_new_status text;
BEGIN
