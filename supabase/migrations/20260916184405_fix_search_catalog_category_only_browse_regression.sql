-- Emergency fix: the previous migration (20260916174805_search_catalog_
-- category_or_text) OR'd the category match with the free-text match to
-- fix parent-category-name search, but the free-text branch auto-passes
-- whenever search_query IS NULL ("search_query IS NULL OR ..."). Category
-- browsing (app/(tabs)/explore.tsx's buildProductsQuery) always calls this
-- RPC with search_query: null, so that OR made the ENTIRE WHERE clause
-- unconditionally true regardless of category_ids -- every category and
-- subcategory browse was silently returning the whole public catalog
-- instead of the selected category. Confirmed live: category_ids scoped to
-- "Hats / caps" alone, search_query null, returned 136 rows (basically the
-- full catalog), only 17 of which actually belonged to that subcategory.
--
-- Fix: only let a real (non-null, non-empty) search_query's text match
-- serve as an alternate path into a category-scoped query. When
-- category_ids is provided, category membership is checked on its own
-- first; browsing with no search text now requires an actual category
-- match again, exactly like before 20260916174805. The text-OR-category
-- behavior for an actual parent-category-name search (both category_ids
-- and search_query populated) is preserved.
...
AS $function$
  ...
  WHERE p.deleted = false
    ...
    AND (
      CASE
        WHEN category_ids IS NOT NULL AND cardinality(category_ids) > 0 THEN
          (
            p.category_id::text = ANY(category_ids)
            OR (
...
REVOKE ALL ON FUNCTION public.search_catalog(text, text[], text[], text[], text[], text[], text[], boolean, boolean, boolean, numeric, numeric, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.search_catalog(text, text[], text[], text[], text[], text[], text[], boolean, boolean, boolean, numeric, numeric, text) TO authenticated, anon;
