-- Fix search_catalog so a parent-category-name search (e.g. "accessories")
-- returns all of that category's subcategory products, not just the ones
-- that happen to also mention the parent name in their own text fields.
--
-- The mobile app expands a typed query into subcategory ids via
-- subCategoryIdsMatching() and passes them as category_ids, but this
-- function previously required category_ids AND the free-text predicate to
-- both match. A Hats product's joined category name is "Hats", not
-- "Accessories", so the text predicate silently excluded it even though
-- category_ids correctly identified it as a match. Changing this to OR
-- means either signal is sufficient. Safe: search_catalog has exactly one
-- caller (app/(tabs)/explore.tsx), and category_ids there is always derived
-- from the same typed search_query, never an independent filter.
...
AS $function$
  ...
  WHERE p.deleted = false
    ...
    -- Category match OR text match (previously AND'd, which silently
    -- excluded subcategory products from parent-category-name searches).
    AND (
      (
      ...
      WHEN search_query IS NOT NULL AND trim(search_query) <> '' THEN
        CASE
          ...
          WHEN length(trim(search_query)) >= 5
            ...
            THEN 8
          -- Category-only match (category_ids resolved it, no text tier hit)
          ELSE 9
        END
      ELSE 0
    END ASC,
