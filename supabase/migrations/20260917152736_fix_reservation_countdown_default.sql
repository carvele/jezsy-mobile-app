-- Migration: 20260917152736_fix_reservation_countdown_default.sql
-- Description: create_reservation_multi's INSERT never set reservations.countdown,
-- so it fell back to the column default (false) on every mobile customer
-- reservation. reservationStatus.ts's "genuine unpaid To Pay" bucket computes
-- showCountdown = countdown !== false && Boolean(payment_due_at) -- with
-- countdown false, showCountdown is always false regardless of whether
-- payment_due_at has actually passed, so the reservation detail screen showed
-- "The payment window has closed" immediately on every new reservation, even
-- though the real deadline (created_at + up to 2h, or 30 min before pickup,
-- whichever is sooner) was still hours away. The admin-assisted POS
-- reservation writer already sets countdown = true explicitly; this brings
-- the customer-facing RPC in line with it. Exact copy of
-- 20260915071500_harden_idempotent_reservation_staff_isolation.sql's
-- create_reservation_multi otherwise -- only the INSERT column list and one
-- value change.
